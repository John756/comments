...
/**
 * Read a NCName value from the reader.
 *
 * @param reader The reader (pointing at a node with a value).
 * @return pointer to the NCName.
 */
static xmlChar *xmlTextReaderReadXsNCNameType(xmlTextReaderPtr reader) {
  return xmlTextReaderReadXsStringType(reader);
}

/**
 * Write a NCName value to the writer.
 *
 * @param writer The writer.
 * @param value The value to be written.
 * @return the bytes written (may be 0 because of buffering) or -1 in case of error.
 */
static int xmlTextWriterWriteXsNCNameType(xmlTextWriterPtr writer, xmlChar *value) {
  return xmlTextWriterWriteString(writer, value);
}

/**
 * Frees a NCName type from memory.
 *
 * @param value The value to free.
 */
static void freeXsNCNameType(xmlChar *value) {
  freeXsStringType(value);
}
